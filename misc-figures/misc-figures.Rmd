...
# All data
data_points <- ggplot() +
  geom_point(data = sim$data, aes(x = plmaf, y = wsmaf), alpha = 0.5) +
  theme_coiaf() +
...
variant <- ggplot() +
  geom_point(data = variant_data, aes(x = plmaf, y = wsmaf), alpha = 0.5) +
  theme_coiaf() +
  labs(x = "PLMAF", y = "Variant Site", title = "Variants")

# Heterozygous data
het_data <- data.frame(wsmaf = sim$data$wsmaf, plmaf = sim$data$plmaf) %>%
  dplyr::filter(wsmaf > 0 & wsmaf < 1)

het <- ggplot() +
  geom_point(data = het_data, aes(x = plmaf, y = wsmaf), alpha = 0.5) +
  theme_coiaf() +
...
variant_final <- ggplot() +
  ...
  annotate(
    ...
    label = c("COI = 3", "COI = 4"), col = "red", size = 7 / .pt
  ) +
  theme_coiaf() +
...
ggsave(
  filename = "~/Desktop/Malaria/manuscript-figures/Fig4.tiff",
  # filename = "/Users/arisp/Desktop/Malaria/coiaf/internal/toverall.png",
  # device = "tiff",
  width = 2000,
  height = 1800,
  units = "px",
...
ggsave("internal/bootstrapped_ci.png", device = "png", width = 2000, height = 1750, units = "px", dpi = "print")
```
